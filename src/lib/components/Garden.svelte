...
<div class="flex flex-wrap gap-3 content-start border min-h-96 rounded-lg p-4  bg-gradient-to-br from-green-50 to-rose-50" >
  {#each gardenCollection as rose (rose.id)}
    ...
    {#if rose.note.length > 0}
      ...
      <div class="flex items-center h-fit min-h-[64px] ml-[52px] min-w-28  break-all p-1 max-w-28 max-h-[64px] bg-surface-50 border hover:shadow-inner border-surface-100 rounded-sm text-xs shadow-sm relative">
        <p>{rose.note}</p>
        <img  class="h-fit  absolute -top-0 -left-12" width="64" height="64" src={rose.type.path} alt="">
      </div>
